...
uint8_t autenticar(void); //esta funci?n compara la contrase?a ingresada con la contrase?a actual, retorna 1 si son iguales, 0 cc
	...
	switch (estado) {
		case CERRADO:
			...
			if (Keyboard_Update(&key)) {
				//si se detecta un n?mero se pasa a abriendo
				if (key >= '0' && key <= '9') {
					buffer_c[0] = key;
					estado = ABRIENDO;
					estado_a = PRIMERO;
		...
		case ABIERTO:
			...
			if (tiempo == 50) { //50 iteraciones en el estado son .5 segundos
				estado = CERRADO;
				tiempo = 0;
			}
			break;
		case DENEGADO:
			...
			if (tiempo == 20) { //2 iteraciones en el estado son .2 segundos
				estado = CERRADO;
				tiempo = 0;
			}
			break;
